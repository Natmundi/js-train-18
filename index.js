...
function parseJson(jsonStr) {
  ...
  try {
    const jsonParc = JSON.parse(jsonStr);
    return jsonParc;
  } catch (error) {
...
function getAge(age) {
  ...
  try {
    if (age < 0) {
      const newError = new Error("Помилка: Вік не може бути менше 0!");
      newError.name = "AgeError";
      throw newError;
    }
  } catch (newError) {
    return { message: newError.message, name: newError.name };
  }
...
function getBookById(books, id) {
  ...
  if (!bookFound) {
    const newError = new Error(`TypeError: Книга з ID ${id} не знайдена!`);
    return newError.message;
  }
  return bookFound;
}
...
function decodeURIComponentWrapper(encodedString) {
  ...
  try {
    ...
    if (error.name === "URIError") {
      return new URIError(`Помилка неправильний URI формат`);
    } else {
